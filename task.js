...
class LastPublishChecker {
  ...
  check() {
    ...
    const minHours = process.env.MIN_PUBLISH_HOURS || 24 * 15 // 根据需要调整
...
const backupGit = () => {
  ...
  try {
    ...
    // 将所有字体格式文件添加到 lfs
    execSync(`git lfs track "*.woff"`, { cwd: __dirname })
    execSync(`git lfs track "*.woff2"`, { cwd: __dirname })
    execSync(`git lfs track "*.ttf"`, { cwd: __dirname })
    execSync(`git lfs track "*.otf"`, { cwd: __dirname })
    execSync(`git lfs track "*.eot"`, { cwd: __dirname })
    ...
    // 提交
    console.log(
      `git commit -m "auto backup on ${now.toISOString()}" && git lfs push origin main`
    )
    const refPushInfo = { hasPushSuccess: false }
